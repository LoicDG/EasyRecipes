...
src/components/                 recipe-card, tag-chip, empty-state, icons, logo
src/constants/theme.ts          Colors (light/dark), Fonts, Spacing, Radius, TagColors
scripts/build-icons.sh          regenerates the launcher/splash PNGs from the mark
...
A spoon standing in a bowl, drawn as a checkmark. Its geometry is duplicated in
three places — `design/logo.svg`, `scripts/build-icons.sh` (raster assets) and
`src/components/logo.tsx` (in-app `LogoMark`) — so change all three together. The
PNGs in `assets/images/` (`icon`, `favicon`, `splash-icon`, `android-icon-*`) are
generated by `scripts/build-icons.sh`, which needs `rsvg-convert` (`brew install
librsvg`); rerun it rather than editing them. The spoon handle's overshoot past the
rim is what makes it a spoon, so don't trim it.
